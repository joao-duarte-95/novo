...
public class Bank {
	...
	public Account accountType(AccountType type, Client client, int amount, int value) throws BankException, AccountException, ClientException {
		Account account;
		switch (type) {
		case CHECKING:
			account = new CheckingAccount(client, amount); break;
		case SAVINGS:
			account = new SavingsAccount(client, amount, value); break;
		case SALARY:
			account = new SalaryAccount(client, amount, value); break;
		case YOUNG:
			account = new YoungAccount(client, amount); break;
		default:
			throw new BankException();
		} return account;
	}
